 %% Snippets of LFP Bootstrapping

% on = 1, off = 2, baseline = 3
function [lfpMat,lfpAvg,spikeAvg] = LFP_Snip_Func_bootstrap(lfp_data,time,LFPstruc,Fs,c,locs2,spikeMat,block,on_off_baseline,lfp_snippet_size,Neuron,smallest_count)
    ii = block;
    i = on_off_baseline;
    ...
    rand_rows = randperm(length(lfpMat(:,1)),smallest_count);
    lfpMat_bootstrap = lfpMat(rand_rows,:);
    
    spikeAvg = mean(spikeMat(CC==Neuron,1:end)); % average action potential waveform
    lfpAvg = mean(lfpMat_bootstrap); % mean lfp waveform
