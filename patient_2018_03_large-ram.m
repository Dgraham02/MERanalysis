...
USER = 'dakota';
file_path = load_data_2018_03(USER);

% Load Data and Header Files 
load(file_path, 'C1'); % neural data
load(file_path, 'Header'); % header file
...
[filtered_hp, filter_params_hp] = filter_high_pass(Fs, sig);

%% Find Stimulation Artifacts 
threshold_art = 400; % threshold just above tallest action potential 
[pks_art, locs_art] = findpeaks(filtered_hp,time,'MinPeakHeight',threshold_art); % MatLab 2018b
...
for i = 1:4
    ...
    plot(time(block(i).indicies(1) : block(i).indicies(2)), ...
         filtered_hp(block(i).indicies(1) : block(i).indicies(2)), ...
         locs_art(locs_art>block(i).markers(1) & locs_art<block(i).markers(2)), ...
         pks_art(locs_art>block(i).markers(1) & locs_art<block(i).markers(2)),'o');
     
    title(stim_names{i})
    xlim([time(block(i).indicies(1)),time(block(i).indicies(2))])
end

%% Find Start/Stop of Stimulation

[start_art, stop_art] = start_stop_times(locs_art);
...
artifact_location_index = (locs_art - time(1)) * Fs;

% Matrix of locations of Artifacts 
artifact_location_matrix = artifact_location_index(:) + artifact_array_blank;

artifact_matrix = filtered_hp(round(artifact_location_matrix)); % Matrix of Artifacts 
...
artZeros = zeros(size(filtered_hp)); % create array of zeros the same length as spikeData 
artZeros(round(art1D)) = filtered_hp(round(art1D)); % create array of artifacts by indexing into spikeData

cleanSpikeData = filtered_hp - artZeros; % subtract artifact from spikeData 
...
threshold = 100; % Choose an appropriate threshold (uV)

APtime = 2/1000; % duration of an action potential = 2ms
[pks_ap_raw, locs_ap_raw] = findpeaks(cleanSpikeData,time,'MinPeakHeight',threshold,'MinPeakDistance',APtime);


%% Plot Marked APs

figure(5); clf;
plot(time,cleanSpikeData, locs_ap_raw, pks_ap_raw,'o')
...
for i = 1:numBlocks
    locs_remove_ends = locs_ap_raw(locs_ap_raw>spike_width & locs_ap_raw<=(length(cleanSpikeData)-spike_width)); % remove spikes too close to edges
    locs_time_adjusted = locs_remove_ends - time(1); % subtract initial time value to match with spiikeData indicies 
    loc_matrix = locs_time_adjusted(:)*Fs + round(spike_array_blank); % Matrix of locations of APs 

    spikeMat = cleanSpikeData(round(loc_matrix)); % Matrix of APs 
end

%% Sort Action Potentials
numClusters = 2;
% PCA to cluster spikes 
[coeff, score] = pca(spikeMat); % PCA
Z = linkage(score(:,1:2),'ward'); % create a tree of hierarchical clusers using 'ward'
c = cluster(Z,'Maxclust',numClusters); % assign spikes to a cluster 
...
subplot(3,numClusters,numClusters+1:numClusters*3)
plot(time,cleanSpikeData)
hold on
scatter(locs_ap_raw, pks_ap_raw, 10, spikeColor(c(:),:))

%% PCA Plot
figure(6); clf;
scatter3(score(:,1),score(:,2),score(:,3),1,spikeColor(c(:),:)); % plot each spike based on the first 3 principle components 
...
neurons = [1]; % selected neurons to use for analysis
...
for i = 1:length(neurons)
    locs_ap = cat(2,locs_ap, locs_ap_raw(c==neurons(i)));
    pks_ap = cat(2,pks_ap, pks_ap_raw(c==neurons(i))); 
end

plot(time,cleanSpikeData)
hold on
scatter(locs_ap, pks_ap,10)
ylim(ylimits)
...
[lfp, filter_params_bp] = filter_band_pass(Fs, sig);
